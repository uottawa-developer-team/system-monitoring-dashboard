...
def plotData(data):
    ...
    for i in range(numOfDisks):
        ...
        capacity = [float(entry['disk'][i]['size'][:-2]) for entry in data]
        availSpace = [float(entry['disk'][i]['avail'][:-2]) for entry in data]
        usedSpace = [float(entry['disk'][i]['used'][:-2]) 
                     if entry['disk'][i]['used'][-2:] == "Gi" else float(entry['disk'][i]['used'][:-2])/1024 
                     for entry in data]
